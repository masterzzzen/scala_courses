...
class Module03 extends FunSpec with Matchers with SeveredStackTraces {
  ...
  // Also provide one concrete method implementation defined only in ReverserTrait called
  // canReverse that returns true
  // Then provide three concrete implementations to satisfy the tests below,
  // uncomment the tests, and make sure they work.
  ...
  class IntReverser extends ReverserTrait {
    type T = Int
    def reverse(s: T): T = throw new IllegalStateException("Int is not reversable")
    def canReverse: Boolean = false
  }
  ...
  trait RangeWeapon {
    ...
    def load(ammunition: SuitableAmmunition): Boolean
  ...
  // A SixShooter, that can be loaded up to 6 times, can only shoot bullets,
  // and can only shoot them while
  // it has ammo left. When out, it will return None rather than Some(Bullet).
  // Weight should be computed as 10
  // plus the weight of the number of bullets. If the gun is full (6 bullets already),
  // do not add the bullet and return
  // false, otherwise return true. Shooting the gun should return a spent bullet and remove
  // it from the ammo list
  class SixShooter extends RangeWeapon {
    type SuitableAmmunition = Bullet
    private[this] var bullets: List[Bullet] = List.empty[Bullet]
    ...
    def shoot: Option[Bullet] =
      if (bullets.nonEmpty) {
        val bullet : Bullet = bullets.head
  ...
  class Crossbow extends RangeWeapon {
    ...
    def shoot: Option[Bolt] =
      if (bolt.isEmpty) {
        None
      } else {
        val saved: Option[Bolt] = bolt
        bolt = None
        saved
      }

    def weight = if (bolt.isEmpty) {
      15
    } else {
      bolt.get.weight + 15
    }

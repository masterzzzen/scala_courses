...
class Module05 extends FunSuite with Matchers with StopOnFirstFailure with SeveredStackTraces {
  ...
  implicit def reversableList[T: Reversable]: Reversable[List[T]] = {
    ts: List[T] =>
      val reverser: Reversable[T] = implicitly[Reversable[T]]
      ts.map(reverser.reverse).reverse
  }
  ...
  test("Create your own intercept method") {
    ...
    def interceptException[T](fn: => Unit)(implicit ct: ClassTag[T]) {
      try {
        fn
        fail("Expected %s exception but didn't get it.".format(ct.runtimeClass.getName))
      }
      catch {
        case NonFatal(th) if th.getClass == ct.runtimeClass =>
        case NonFatal(th) =>
          fail("Expected %s exception but got %s exception.".format(ct.runtimeClass.getName, th.getClass.getName))
      }
    }
  ...
  test("Implicit parameter example") {
    ...
    object Eventually {
      ...
      def eventuallyWith(maxTries: Int, interval: Int)(f: => Unit): Unit = {
        ...
        while (!succeeded && numFailed < maxTries) {
          optEx =
            try {
              f
              None

...
class Module04 extends FunSpec with Matchers with SeveredStackTraces {
  ...
  // using structural typing, define a method sameLength which takes 2 of any kind of object that
  // defines length, returning
  // true if they are the same length, false otherwise. If you do it right, we should be able to
  // compare a string
  ...
  describe ("Using refinement types") {
    ...
    val phFruits: ListBuffer[Fruit{def ph: Int}] = mutable.ListBuffer.empty[Fruit { def ph: Int }]

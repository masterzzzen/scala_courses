...
class Module01 extends KoanSuite with Matchers {
  ...
  class CalorieTracker {
    private[this] var dailyMax_ : Int = 0
    def dailyMax: Int = dailyMax_
    def dailyMax_=(x: Int) = {
      require(x > 0 && x < 5000,
        "the dailyMax must be larger than 0 and smaller than 5000 calories")
      dailyMax_ = x
    ...
    private[this] var currentDaily_ : Int = 0
    def currentDaily: Int = currentDaily_
    def currentDaily_=(x: Int) = {
      require(x > -500 && x < (dailyMax_ + 500),
        "the currentDaily may only be set to values in a range from -500 to dailyMax + 500")
      currentDaily_ = x
    }
  ...
  object DBCache {
    val recipeCache: LoadingCache[String, Future[List[String]]] = CacheBuilder.newBuilder().build(
      new CacheLoader[String, Future[List[String]]] {
        def load(key: String): Future[List[String]] = Future(SlowDB.findRecipes(key))
      }
    )

...
class Module02 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test("Can create an empty Car of a certain type with Car.empty[TYPE]") {
    ...
    object Car {
      def empty[T <: TransportableItem]: Car[T] = new Car[T]
    }
  ...
  // of the the put method. This Car is an immutable object, so add returns a new Car,
  // potentially of a more general type:
  //
  ...
  //
  // We suggest you hold on to items in a car with a Vector, as this makes updating items
  // at the end
  ...
  test("Empty Car should store items of the right type and access them by index") {
    ...
    class Car[T <: TransportableItem](items: List[T]) {
      def add(x: T): Car[T] = new Car(items :+ x)
      def numberOfItems: Int = items.size
      def item(n: Int): Option[T] = items.lift(n)
    }

    object Car {
      def empty[T <: TransportableItem]: Car[T] = new Car[T](List.empty)
    }

    val emptyCar: Car[Cargo] = Car.empty[Cargo]
  ...
  test("Car should be covariant in its type parameter") {
    ...
    object Car {
      private class CarImpl[+T <: TransportableItem](val items: List[T]) extends Car[T] {
        def add[U >: T <: TransportableItem](x: U): Car[U] = new CarImpl(items :+ x)
        def numberOfItems: Int = items.size
        def item(n: Int): Option[T] = items.lift(n)
      }
      def empty[T <: TransportableItem]: Car[T] = new CarImpl[T](List.empty)
    }

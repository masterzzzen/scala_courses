...
class Module10Json extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  implicit object ItemJsonFormat extends Format[Item] {
    ...
    override def reads(json: JsValue): JsResult[Item] = {
      val opt: Option[Item with Product with Serializable] =
        json.asOpt[Book]
          .orElse(json.asOpt[Journal])
          .orElse(json.asOpt[DVD])
          .orElse(json.asOpt[CD])
      opt.map(item => JsSuccess(item))
        .getOrElse(JsError("Could not parse Item from JSON"))
    }
  }

...
class Module09 extends FunSpec with Matchers with SeveredStackTraces with CancelAfterFailure {
  ...
  describe ("RoShamBo") {
    ...
    def rock(play: RoShamBo): Outcome = {
      play match {
        case Rock => Draw
        case Paper => Lose
        case Scissors => Win
    ...
    def paper(play: RoShamBo): Outcome = {
      play match {
        case Rock => Win
        case Paper => Draw
        case Scissors => Lose
    ...
    def scissors(play: RoShamBo): Outcome = {
      play match {
        case Rock => Lose
        case Paper => Win
        case Scissors => Draw
  ...
  describe ("Process the first line of a file with a function") {
    ...
    // try .trim()
    def withFileContents[T](file: File)(fn: String => T): T = {
      val fileReader: BufferedReader = new BufferedReader(new FileReader(file))
      try {
        val line: String = fileReader.readLine.trim
        fn(line)
      }
      finally {
        fileReader.close()

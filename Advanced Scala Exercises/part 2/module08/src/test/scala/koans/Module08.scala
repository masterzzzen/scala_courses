...
class Module08 extends FunSuite with Matchers with StopOnFirstFailure with SeveredStackTraces {
  ...
  test ("Get deep list of optional fields") {
    ...
    // in the carriages there will be a list of passengers, and each passenger may, or may not,
    ...
    def allCellNumbersForPassengers(routes: Seq[Route]): Seq[String] =
      for {
        ...
        carriage <- activeTrain.carriages
        passenger <- carriage.passengers
        cellPhoneNumber <- passenger.cellPhoneNumber
  ...
  test("Count the vowels") {
    ...
    def countVowels(str: String): Int = {
      ...
      str.count(Set('a','e','i','o','u','A','E','I','O','U'))
    }
  ...
  object MaybeError {
    def Error[Nothing](error: String): MaybeError[Nothing] = {
      new MaybeError(Left(error))
    }

    def Value[B](value: B): MaybeError[B] = {
  ...
  def isGood(numberRow: Seq[Int]): Boolean = {
    numberRow match {
      case nums if nums.distinct.size != 4 => false
      case nums if nums.exists { num => num > 4 || num < 1 } => false
      case _ => true
    }
  }
  ...
  def transpose(matrix: Seq[Seq[Int]]): Seq[Seq[Int]] = {
    matrix match {
      case Nil => Nil
      case _ if !matrix.tail.forall(_.size == matrix.head.size) => throw new IllegalArgumentException("Irregular shaped matrix, must be rectangular")
      case _ => transposeHelper(matrix: Seq[Seq[Int]])
    }
  }

  def transposeHelper(matrix: Seq[Seq[Int]]): Seq[Seq[Int]] = {
  ...
  def group2x2(board: Seq[Seq[Int]]): Seq[Seq[Int]] = {
    require(board != Nil, "Board should not be Nil")
    require(board.forall(_.size == 4), "Every row should have 4 columns")
    require(board.size == 4, "The board should have 4 rows")

...
class Module07 extends FunSuite with Matchers with SeveredStackTraces {
  ...
  test ("Numerology of hosts") {
    ...
    val hostList = List("localhost",
      "www.cnn.com", "www.slashdot.org", "SomeTotallyMadeUpHostNameXXYYZZ.com")
  ...
  test ("Inject using Parfait") {
    ...
    class SiteLookerUpper(implicit config: DBConfig) {
      val db = config.db
    ...
    class DBNumerologer(implicit config: SystemConfig) {
    ...
    implicit object SystemConfigObject extends SystemConfig {

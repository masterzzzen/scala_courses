...
class Module11 extends FunSpec with Matchers with SeveredStackTraces {
  ...
  describe ("Calculating Pi") {
    ...
    class CalcPi {
      ...
      def draw(iterations: Int): Future[Double] = {
      ...
      def draws(n: Int, iterations: Int): Future[Vector[Double]] = {
        val drawVec = (for (_ <- 1 to n) yield draw(iterations)).toVector
        Future.sequence(drawVec)
      }
      ...
      def calc(n: Int, iterations: Int): Future[Double] = {
        for ( xs <- draws(n, iterations)) yield (xs.sum / xs.length)
      }
    }

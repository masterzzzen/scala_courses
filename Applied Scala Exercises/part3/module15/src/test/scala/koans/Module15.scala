...
class Module15 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("Apply function to list in Java") {
    ...
    val l2 = obtuse.doMathFuncOnList(l1, new ObtuseJavaLib.MathFunc {
      override def apply(in: Int) = in * in
    })
  ...
  test ("Apply closures directly using the doMathFuncOnList method") {
    ...
    def applyFunc(s: Seq[Int], fn: Int => Int): List[Integer] = {
      val integerList = s.toList.map { new java.lang.Integer(_) }
      val mathFunc = new ObtuseJavaLib.MathFunc {
        def apply(in: Int): Int = fn(in)
      }
      obtuse.doMathFuncOnList(integerList.asJava, mathFunc).asScala.toList
    }

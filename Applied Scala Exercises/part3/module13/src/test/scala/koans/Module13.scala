...
class Module13 extends FunSuite with Matchers with StopOnFirstFailure with SeveredStackTraces {
  ...
  def comboSetsForSequences(sequences: List[String]): List[Set[Char]] = {
    val s1 = stringToSetOfChars(sequences.head)
    sequences.tail.foldLeft(s1) { combineZippedSetsAndString(_, _) }
  }
  ...
  def partitionByFirst(numbers: List[Int]): (Int, List[Int], List[Int]) = {
    val smaller = numbers.tail.filter(x => x < numbers.head)
    val remaining = numbers.tail.filter(x => x >= numbers.head)
    (numbers.head, smaller, remaining)
  }
  ...
  def mysteryFunction(numbers: List[Int]): List[Int] = {
    numbers match {
      case Nil => Nil
      case _ =>
        val result = partitionByFirst(numbers)
        mysteryFunction(result._2) ::: List(result._1) ::: mysteryFunction(result._3)
    }
  }

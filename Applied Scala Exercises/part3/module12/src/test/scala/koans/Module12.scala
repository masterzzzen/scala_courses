  ...
  def applyTransaction(transaction: Transaction) =
    transaction match {
      ...
      case OpenAccount(accountType, accountNum) =>
        if (accountsMapForType(accountType).contains(accountNum)) {
      ...
      case Withdrawal(accountType, accountNum, _)
        if !accountsMapForType(accountType).contains(accountNum) =>
          throw new NoSuchElementException("Account doesn't exist")

      case Withdrawal(Checking, accountNumber, amount) =>
        if (getAccount(Checking, accountNumber).balance + 1000 < amount ) {
         throw new IllegalStateException("Checking account is already overdrawn")
        } else {
          getAccount(Checking, accountNumber).withdrawal(amount)
        }

      case Withdrawal(Savings, accountNumber, amount) =>
        if (getAccount(Savings, accountNumber).balance < amount) {
          throw new IllegalStateException("Savings account is already overdrawn")
        } else {

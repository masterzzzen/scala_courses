...
class Module04 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("Refactoring var to val") {
    ...
    def max(numbers : List[Int]) = {
      ...
      def max_rec(theMax: Int, rest: List[Int]): Int = rest match {
        case Nil => theMax
        case n::tail =>
          val newMax = if (n > theMax ) n else theMax
          max_rec(newMax, tail)
      }

      max_rec(Int.MinValue, numbers)
    }
  ...
  def listShakespeareFiles(dirPath: String) : Array[String] = {
    ...
    for (file <- fileList
      if file.getName.endsWith(".shkspr")) yield file.getName
  }
  ...
  test ("shakespeare files contain question or statement?") {
    ...
    val shakespeareFilenames = listShakespeareFiles(".").toList
    for (file <- shakespeareFilenames) {
      shksprMap.update(file, statementOrQuestion(firstLineOfFile(file)))
    }

...
class Module05 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("Multiply numbers") {
    var multiplier = 3
    val mult: Int => Int = num => num * multiplier
  ...
  test ("Function with placeholder syntax") {
    ...
    val mult: (Int, Int) => Int = (_: Int) * (_: Int)
  ...
  test ("Multiply variable number of arguments") {
    ...
    // to ensure it works
    def multiplyDoubles(doubles: Double*): Double = doubles.toList match {
      case Nil => 1.0
      case head::tail => head * multiplyDoubles(tail: _*)
    }
  ...
  test ("Recurse with varargs") {
    ...
    def listOfLists(strs: String*): List[List[String]] = strs match {
      case Nil => Nil
      case _ => strs.toList :: listOfLists(strs.tail: _*)
    }
  ...
  test ("URL cleaner") {
    def urlClean(urlAsString : String) : URL = {
      urlAsString match {
        case goodUrl if urlAsString.startsWith("http://") => new URL(goodUrl)
        case _ =>  new URL("http://badurl.com")
      }
    }

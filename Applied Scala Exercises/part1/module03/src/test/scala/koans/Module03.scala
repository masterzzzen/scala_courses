...
class Module03 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  class ComplexNum(val real: Double, val imaginary: Double){
    ...
    override def toString: String = {
      if (this.imaginary == 0.0) this.real.toString
      else if (this.imaginary > 0.0) s"${ this.real.toString } + ${ this.imaginary.toString }i"
      else s"${ this.real.toString } - ${ math.abs(this.imaginary).toString }i"
    }
    def +(other: ComplexNum): ComplexNum = new ComplexNum(this.real + other.real, this.imaginary + other.imaginary)
    def +(other: Double): ComplexNum = new ComplexNum(this.real + other, this.imaginary)
  }

  object ComplexNum {
    implicit def apply(r: Double): ComplexNum = new ComplexNum(r)
  }

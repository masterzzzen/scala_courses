...
class Module06 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("Detect any odds/evens in a list") {
    ...
    def containsOdd(nums: List[Int]): Boolean = nums.exists(x => x % 2 == 1)
    def containsEven(nums: List[Int]): Boolean = nums.exists(x => x % 2 == 0)
  ...
  test ("With file contents control structure") {
    ...
    // try .trim()
    def withFileContents(filePath: String)(fn: String => String): String = {
      val file = new BufferedReader(new FileReader(filePath))
      try {
        fn(file.readLine.trim)
      } finally {
        file.close()
      }
    }
  ...
  test ("onlyIfTrue - your own predicate guard") {
    ...
    // return (Unit).
    def onlyIfTrue(predicate: => Boolean)(fn: => Unit) = {
      if (predicate) fn
    }

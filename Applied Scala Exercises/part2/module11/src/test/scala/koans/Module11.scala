...
class Module11 extends FunSpec with Matchers with MockFactory with PropertyChecks {
  ...
  describe ("The loan pattern for a file") {
    ...
    it ("should call a function with the known contents of a file") {
      ...
      (mockFileReader.close _: () => Unit).expects().returning()
      (mockFileReader.nextLine _: () => String).expects().returning("To be or not to be")
    ...
    // Next, using a mock make the nextLine call result in a NoSuchElementException("next on empty iterator") being thrown
    ...
    it ("should always call close() on the file, even if the file is empty causing a NoSuchElementException") {
      val mockFileReader = mock[FileReader]
      (mockFileReader.close _: () => Unit).expects().returning()
      (mockFileReader.nextLine _: () => String).expects().throwing(new NoSuchElementException("next on empty iterator"))
